...
# 実際のツール導入状況に依存しないよう、tools.tsv の check 欄に cmd:true/file:does-not-exist を置いて検査する。
set -uo pipefail
...
setup
add_tool alpha must cmd:true - -
add_tool beta recommended cmd:true fallback-beta -
...
setup
add_tool alpha must file:does-not-exist - context7-id-alpha
add_tool beta recommended cmd:true fallback-beta -
out="$(cd "$WORK" && "$TARGET" 2>&1)"; rc=$?
...
setup
add_tool alpha must cmd:true - -
add_tool beta recommended file:does-not-exist fallback-beta -
out="$(cd "$WORK" && "$TARGET" 2>&1)"; rc=$?
expect_exit "exit 0 を返す" 0 "$rc"
...
printf 'gamma\tmust\tfile:does-not-exist\t-\t-\n' >> "$WORK/.ai/profiles/other/tools.tsv"
add_tool alpha must cmd:true - -
...
setup
printf '# tool\tclass\tcheck\tfallback\tdocs\r\n' > "$WORK/.ai/profiles/testprofile/tools.tsv"
printf 'alpha\tmust\tcmd:true\t-\t-\r\n' >> "$WORK/.ai/profiles/testprofile/tools.tsv"
printf 'beta\trecommended\tfile:does-not-exist\tfallback-beta\t-\r\n' >> "$WORK/.ai/profiles/testprofile/tools.tsv"
out="$(cd "$WORK" && "$TARGET" 2>&1)"; rc=$?
...
setup
printf 'docker\tmust\t\tcmd:true\tcontext7-docker\n' >> "$WORK/.ai/profiles/testprofile/tools.tsv"
out="$(cd "$WORK" && "$TARGET" 2>&1)"; rc=$?
...
echo "case 12: 未知の check 語彙は exit 2"
setup
add_tool alpha must unknown:payload - -
out="$(cd "$WORK" && "$TARGET" 2>&1)"; rc=$?
expect_exit "exit 2 を返す" 2 "$rc"
expect_contains "check 列の不正を示す" "check 列が不正" "$out"
teardown
