...
- [x] work unit schema が version、分類、profile、model、token、時間、outcome、retry、review、rework、quality gate、artifact link を検証する。
- [x] token 取得不能を `null` として保存し、0 と区別して欠測率を report する。
- [x] schema 外 field、自由記述、secret / 個人情報を意図する field、絶対 path、親 directory traversal を拒否する。
- [x] 1 work unit = 1 JSON file で atomic に作成し、duplicate ID を上書きしない。
- [x] work unit、Issue、PR、release、profile の各粒度で Markdown と JSON report を決定的に生成できる。
- [x] 欠測条件が揃わない cohort を `partial`, `unavailable`, `not_comparable` として区別する。
- [x] local-only、既存 project Git 追跡、専用 Git repository の 3 モードを検証する。
- [x] 既存 Git repository がない場合、metrics root を local Git repository として初期化する。
- [x] 既存 project に統計を commit / push しうる mode は明示 acknowledgement と public / unknown remote warning を要求する。
- [x] AF は既存 repository、remote、commit、push を自動操作せず、保存先に Git repository がない場合の metrics root の local `git init` だけを行う。
- [x] AF 開発の実測値は AF 本体とは別の private 計測 repository に置き、一般利用者へ access させないことを文書化する。
- [x] 一般利用者は新規 repository を必須とせず、既存 project repository または自身の専用 repository を選べる。
- [x] `.agentic-framework/installation.json` が `managed`, `seeded`, `local-data` と checksum を保持する。
- [x] AF update scope check が `managed` 以外、利用者変更済み managed file、application source、metrics data の混入を非 0 で拒否する。
- [x] metrics 未記録や remote 未設定は warning に留め、task completion を block しない。
- [x] 合成 fixture だけで代表 report と failure case を検証する。
- [x] 外部 package を追加せず、既存 test と docs link check が回帰しない。
- [x] 実装、検証 command、既知制約を work note に記録する。
