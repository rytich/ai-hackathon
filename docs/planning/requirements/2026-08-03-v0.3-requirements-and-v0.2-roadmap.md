...
# v0.3 要件定義と v0.2 ロードマップ

> Status: approved by merge of PR #48
>
...
| R7 | 定期自動改善の統制 | hermes 等の loop がリファクタリング・保守・検証を継続実行しても、1 repo 1 knowledge base と人間承認境界を崩さない。 |
...
| v0.2.7 | 定期自動改善ループ | #30 | hermes 等が定期的にリファクタリング・保守・検証を回す runbook、役割分担、quality gate、approval boundary を定義し、dry-run で検証する。 |
| v0.2.8 | 環境横断の採用検証 | #49 | Codex、Claude Code、hermes の少なくとも 2 環境で bootstrap から完了パイプラインまでを再現検証する。 |
| v0.2.9 | v0.3 release candidate | #50 | v0.3 要件 R1-R7 のトレーサビリティ、upgrade 手順、既知制約、release candidate の品質ゲートを確認する。 |
...
| #30 | v0.2.7 | harness（#31）と計測（#34）を前提に、hermes 等が定期的なリファクタリング・保守・検証を安全に回す自動化レイヤーを定義する。 |
...
## 8. 実装フェーズの follow-up 判断

1. #49 で検証対象にする 2 環境以上を Codex / Claude Code / hermes から選ぶ。
2. #50 で v0.2.9 を RC とするか、v0.3.0 へ直接進むかを人間が判断する。
3. #38 を AF repository から分離する時期と追跡先は、公開ページの文面承認と合わせて判断する。
