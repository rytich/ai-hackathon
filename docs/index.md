# docs マップ

「〜したい」→「どこを見る / どこに書く」の索引。構造とスキーマの正本は [framework/knowledge-base.md](framework/knowledge-base.md)。

## やりたいこと別

| やりたいこと | 場所 |
|---|---|
| エージェント運用のルールを知る | [framework/](framework/README.md) |
| docs 全体の運用規約を知る | [README.md](README.md) / [framework/knowledge-base.md](framework/knowledge-base.md) |
| AF を他プロジェクトに新規導入する | [framework/project-adoption.md](framework/project-adoption.md) |
| 導入済みプロジェクトの AF を最新化する | [framework/project-update.md](framework/project-update.md) |
| AI 作業の効果を記録・集計する | [framework/effect-metrics.md](framework/effect-metrics.md) |
| 説明サイトを公開・更新する | [framework/site-hosting.md](framework/site-hosting.md) |
| ツールチェーン（superpowers/SpecKit/crit/skills/Issues/Linear）の標準フロー | [framework/toolchain-flow.md](framework/toolchain-flow.md) |
| どの AI にどのタスクを任せるか決める | [framework/ai-environment-profiles.md](framework/ai-environment-profiles.md) |
| 指示の書き方（What/Why/How）を知る | [framework/collaboration-rules.md](framework/collaboration-rules.md) |
| 企画・差分にレビューを返す | [framework/collaboration-rules.md](framework/collaboration-rules.md) |
| 調査メモを残す / 読む（情報ソース付き） | [planning/research/](planning/research/) |
| 第1〜4回 AI Agent Hackathon の提出・受賞作品を調査する | [planning/research/hackathons/](planning/research/hackathons/README.md) |
| 要件定義を残す / 読む | [planning/requirements/](planning/requirements/) |
| なぜそう決めたかを残す / 読む | [decisions/](decisions/) |
| 作業サマリーを残す / 読む | [work-notes/](work-notes/) |
| プロダクト仕様・ロードマップ | [knowledge/product/](knowledge/product/) |
| アーキ・runtime・secret・テスト方針 | [knowledge/engineering/](knowledge/engineering/) |
| ビジネスモデル・価格・提供形態 | [knowledge/business/business-model/](knowledge/business/business-model/) |
| マーケティング施策 | [knowledge/business/marketing/](knowledge/business/marketing/) |
| ブランディング（トーン・ロゴ・命名） | [knowledge/business/branding/](knowledge/business/branding/) |
| 競合調査 | [knowledge/business/competitive/](knowledge/business/competitive/) |
| 法務・規制・プライバシー | [knowledge/business/legal/](knowledge/business/legal/) |
| FAQ・カスタマーサポート | [knowledge/support/](knowledge/support/) |
| 説明資料・ピッチ・対外資料 | [knowledge/materials/](knowledge/materials/) |

## レイヤーの考え方

- **framework/** = 運用ルール（メタ）
- **knowledge/** = 今の事実（stock）
- **planning/** = 企画中の成果（flow）
- **decisions/ · work-notes/** = 履歴（log）

企画（調査・要件）と実作業（実装）は分ける。詳細は [framework/knowledge-base.md](framework/knowledge-base.md#企画と実作業の分離)。
