...
- まだ資料なし。作成したら `[タイトル](ファイル名.md)` で追記する。
...
repository が private でも、資料や成果物は外部へ配布することがある。**repository の中身をそのまま渡さない。** 内部の作業記録や、他プロジェクト・顧客の名前が含まれるため。

配布用 archive は必ず次で作る。

```bash
scripts/build-public-archive.sh v<version>
```
